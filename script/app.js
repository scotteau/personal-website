window.addEventListener("load", () => {
  ...
  const aboutMe = new ScrollMagic.Scene({
    ...
    duration: 400,
    triggerHook: 0.8
  }).addIndicators({name: "highlighter"}).setTween(highlighter1).addTo(controller);

  const work = new ScrollMagic.Scene({
    triggerElement:".work h2",
    duration: 400,
    triggerHook: 0.8
  }).addIndicators({name: "highlighter"}).setTween(highlighter2).addTo(controller);

  const contact = new ScrollMagic.Scene({
    triggerElement:".contact h2",
    duration: 400,
    triggerHook: 0.8
  }).addIndicators({name: "highlighter"}).setTween(highlighter3).addTo(controller);
//endregion
});
